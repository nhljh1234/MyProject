...
vec3 getDirLight (DirLight light, vec3 normal, vec3 viewDir) 
{
    ...
    ambientLight = light.ambient * vec3(texture(texture_diffuse_1, texCoords));

    vec3 lightDir = normalize(-light.direction);
    
    float diff = max(dot(normal, lightDir), 0.0);
    diffuseLight = diff * light.diffuse * vec3(texture(texture_diffuse_1, texCoords));

    vec3 reflectDir = reflect(-lightDir, normal);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
    specularLight = spec * light.specular * vec3(texture(texture_specular_1, texCoords));
...
vec3 getPointLight (PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    ambientLight = light.ambient * vec3(texture(texture_diffuse_1, texCoords)) * F;

    vec3 lightDir = normalize(light.position - fragPos);
    
    float diff = max(dot(normal, lightDir), 0.0);
    diffuseLight = diff * light.diffuse * vec3(texture(texture_diffuse_1, texCoords)) * F;

    vec3 reflectDir = reflect(-lightDir, normal);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
    specularLight = spec * light.specular * vec3(texture(texture_specular_1, texCoords)) * F;
...
vec3 getSpotLight(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    {   
        return light.ambient * vec3(texture(texture_diffuse_1, texCoords)) * F;
    }
    float ratio = 1.0f;
    ...
    float diff = max(dot(normal, lightDir), 0.0);
    diffuseLight = diff * light.diffuse * vec3(texture(texture_diffuse_1, texCoords)) * F * ratio;

    vec3 reflectDir = reflect(-lightDir, normal);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
    specularLight = spec * light.specular * vec3(texture(texture_specular_1, texCoords)) * F * ratio;
