...
public class TowerCraneRunData {
    ...
    //判断上下行，根据高度值对应
    public AppGlobalData.Status_Up_Or_Down getUpDownStatus() {
        if (oldData != null) {
            if (height > oldData.height) {
                return AppGlobalData.Status_Up_Or_Down.Up;
            } else if (height < oldData.height) {
                return AppGlobalData.Status_Up_Or_Down.Down;
            }
        }
        return AppGlobalData.Status_Up_Or_Down.Normal;
    }

    //判断左转还是右转，根据回转值对应
    public AppGlobalData.Status_Left_Or_Right getLeftRightStatus() {
        if (oldData != null) {
            if (turnAround > oldData.turnAround) {
                return AppGlobalData.Status_Left_Or_Right.Right;
            } else if (turnAround < oldData.turnAround) {
                return AppGlobalData.Status_Left_Or_Right.Left;
            }
        }
        return AppGlobalData.Status_Left_Or_Right.Normal;
    }

    //判断前进还是后退，根据幅度值对应
    public AppGlobalData.Status_Front_Or_Back getFrontBackStatus() {
        if (oldData != null) {
            if (amplitude > oldData.amplitude) {
                return AppGlobalData.Status_Front_Or_Back.Front;
            } else if (amplitude < oldData.amplitude) {
                return AppGlobalData.Status_Front_Or_Back.Back;
            }
        }
        return AppGlobalData.Status_Front_Or_Back.Normal;
    }
