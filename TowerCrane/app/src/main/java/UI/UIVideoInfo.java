...
public class UIVideoInfo implements InterfaceUI, SurfaceHolder.Callback {
    ...
    final Handler handler = new Handler() {
        public void handleMessage(Message msg) {
            if (msg.obj == AppGlobalData.Handler_Type.SearchFinish) {
                updateVideoList();
                if (videoManager.getVideoList().size() == 0) {
                    textTest.setText("没有摄像头");
                }
            } else if (msg.obj == AppGlobalData.Handler_Type.IPError) {
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        if (surface.isValid()) {
            if (!Player.getInstance().setVideoWindow(videoDataSelect.getIPort(), 0, holder)) {
                Log.e(AppGlobalData.LogTag, "surfaceCreated Player setVideoWindow failed!");
            }
        }
    }
