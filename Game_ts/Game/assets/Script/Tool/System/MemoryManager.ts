    ...
    export function memoryCheck() {
        if (getNowSpriteUseMemory() > HistoryGame.Game.MAX_MEMORY_NUM) {
            clearMemory();
        }
    }
    ...
    export function clearMemory() {
        if (!HistoryGame.Game.AUTO_CLEAR_MEMORY_FLAG) {
            return;
        }
        let prefabSave = HistoryGame.PrefabManager.prefabSave;
        let clearArr = [];
        let allPrefabArr = [];
        //这个是不可以清除的预制体数组
        let canNotClearArr = [];
        ...
        if (HistoryGame.Game.USE_STRONG_CLEAR_MODE) {
            HistoryGame.PrefabManager.clearPrefabInStrongMode(clearArr, canNotClearArr);
            HistoryGame.SpriteFrameManager.clearDestroyNode();
            return;
        }
        //按frequency从小到大排序
        ...
        while (true) {
            ...
            clearArr.forEach((onePrefabData) => {
                if (HistoryGame.PrefabManager.judgeCanClearPrefab(onePrefabData, canNotClearArr)) {
        ...
        HistoryGame.PrefabManager.clearPrefabWithoutUseSpreite(clearArr);
        HistoryGame.SpriteFrameManager.clearDestroyNode();
    }